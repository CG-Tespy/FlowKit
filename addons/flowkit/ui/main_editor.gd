...
func _delete_selected_item() -> void:
	...
	if item_to_delete is FKConditionBlockUi:
		...
		var act_data := item_to_delete.get_block() as FKActionBlock
		var event_data = parent_row.get_event_data()
...
func _finalize_event_creation(inputs: Dictionary) -> void:
	...
	data.actions = [] as Array[FKActionBlock]
...
func _replace_event(expressions: Dictionary) -> void:
	...
	new_data.actions = old_data.actions if old_data else ([] as Array[FKActionBlock])
...
func _on_branch_add_else(branch_item, event_row) -> void:
	...
	var else_data = FKActionBlock.new()
	...
	else_data.branch_actions = [] as Array[FKActionBlock]
...
func _finalize_branch_creation(inputs: Dictionary) -> void:
	...
	var branch_data = FKActionBlock.new()
	...
	branch_data.branch_actions = [] as Array[FKActionBlock]
...
func _finalize_elseif_creation(inputs: Dictionary) -> void:
	...
	var elseif_data = FKActionBlock.new()
	...
	elseif_data.branch_actions = [] as Array[FKActionBlock]
...
func _on_action_dropped(source_row, action_data: FKActionBlock, target_row) -> void:
