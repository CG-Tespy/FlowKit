...
func copy_action(action_data: FKActionBlock) -> void:
	clear()
	_type = "action"
...
func paste_action() -> Array[FKActionBlock]:
	if _type != "action":
		return []
	var result: Array[FKActionBlock] = []
...
func _serialize_action(act: FKActionBlock) -> Dictionary:
	var dict = {
...
func _deserialize_condition(dict: Dictionary) -> FKEventCondition:
	...
	cond.actions = [] as Array[FKActionBlock]  # Always empty for conditions
	return cond


func _deserialize_action(dict: Dictionary) -> FKActionBlock:
	...
	if act.is_branch:
		...
		act.branch_actions = [] as Array[FKActionBlock]
