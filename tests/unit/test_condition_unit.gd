...
# ---------------------------------------------------------
func test_condition_unit_basic_serialization():
...
# ---------------------------------------------------------
func test_condition_unit_roundtrip():
	var cond := FKConditionUnit.new()
...
# 3. Deep duplication
# ---------------------------------------------------------
func test_condition_unit_deep_duplication():
	var cond := FKConditionUnit.new()
	...
	assert_true(deep_copy_success, "Duplicate should be a different instance")

	# Inputs deep-copied
	deep_copy_success = not is_same(dup.inputs, cond.inputs)
	assert_true(deep_copy_success, "Inputs dictionary should be deep-copied")
	...
	dup.inputs["x"] = 999
	assert_ne(cond.inputs["x"], dup.inputs["x"], "Mutating duplicate should not affect original")
