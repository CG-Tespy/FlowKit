...
FKEventSheet
  └─ events: Array[FKEventBlock]
      ...
      └─ actions: Array[FKActionBlock] (executed sequentially)
```
...
Branch data is stored on `FKActionBlock` resources via `branch_id: String` and `branch_inputs: Dictionary`. Legacy `branch_type` values ("if"/"elseif"/"else") are resolved to `"if_branch"` via `registry.resolve_branch_id()` for backward compatibility.
